...
Azure Private Endpoint を使えば、Azure PaaS サービスへの通信を VNet 内のプライベート IP 経由に閉じることができます。しかし、Private Endpoint を作っただけでは不十分で、**DNS の名前解決が正しく Private IP を返すように構成する**必要があります。

ここで登場するのが Azure Private DNS Zone です。Private DNS Zone に A レコードを登録し、対象の VNet にリンクすることで、VNet 内の VM やサービスが Private Endpoint の Private IP を名前解決できるようになります。

シンプルな仕組みに見えますが、実務では「名前解決ができない」「なぜかパブリック IP に解決される」「オンプレからだけつながらない」といったトラブルが頻発します。本記事では、実際にハマりやすい 7 つのパターンを事象・原因・解決策の流れで整理します。
...
- **`-EnableRegistration` は `$false`** にしてください。`$true` にすると自動登録（VM のホスト名を DNS に登録する機能）が有効になりますが、Private Endpoint 用途では不要です。
...
Azure Private DNS Zone はあくまで **リンクされた VNet 内** でのみ有効です。オンプレミスの DNS サーバーは Azure Private DNS Zone を参照できません。そのため、オンプレミスのクライアントはパブリック DNS を引いてしまい、パブリック IP が返ります。

### 解決策

**DNS フォワーダー（条件付きフォワーダー）** を構成して、`privatelink.*.core.windows.net` などのクエリを Azure DNS（168.63.129.16）に転送します。
...
- 特に **Azure Monitor 関連** は `privatelink.oms.opinsights.azure.com`、`privatelink.ods.opinsights.azure.com`、`privatelink.agentsvc.azure-automation.net`、`privatelink.monitor.azure.com`、`privatelink.blob.core.windows.net` の **5 つのゾーンが必要** で、漏れやすいです。
...
VNet または VM の **DNS 設定がカスタム DNS サーバーを指している**場合、Azure 既定の DNS（168.63.129.16）が使われず、Private DNS Zone が参照されません。カスタム DNS サーバーが `privatelink.*` のクエリをパブリック DNS に転送してしまっていることが原因です。
...
- VNet の DNS 設定を「既定（Azure 提供）」に戻すと問題は解消しますが、Active Directory 環境ではカスタム DNS が必要なため、条件付きフォワーダーで対応するのが現実的です。
...
この場合、Private DNS Zone は **Hub VNet にのみリンク**されていれば十分です（168.63.129.16 は Hub VNet のコンテキストで Private DNS Zone を参照するため）。

### ポイント

- Hub-Spoke が多数ある環境では、**パターン B の中央集約型 DNS 設計**のほうが管理しやすいです。すべての Spoke VNet にリンクを貼る必要がなくなります。
- Azure DNS Private Resolver を Hub に配置し、Spoke VNet の DNS 設定でその IP を指定する構成が推奨されています。
...
- `nslookup` の結果だけで「DNS は問題ない」と判断しないことが重要です。**`Resolve-DnsName`（Windows）や `dig`（Linux）で OS の名前解決と同じ結果になるか**を確認しましょう。
...
1. **`Resolve-DnsName` で FQDN を引いて、Private IP が返るか？**
   - 返らない → DNS 構成の問題（パターン 1〜5, 7）
   - 返る → ネットワークまたはアプリ側の問題（パターン 6）
2. **CNAME チェーンに `privatelink` が含まれるか？**
   - 含まれない → Private Endpoint の構成自体を確認
   - 含まれる → Private DNS Zone 側の問題（リンク、レコード、ゾーン名）
3. **どこから名前解決しているか？**
   - VNet 内 → Private DNS Zone のリンクとレコードを確認
   - オンプレミス / 他 VNet → DNS フォワーダーの構成を確認

このフローに沿って切り分ければ、大半の問題は特定できます。
